...
export class MailService {
  ...
  async sendDriverAssignedEmail(driverEmail: string, driverName: string, booking: any) {
    ...
    const subject = `New booking assigned: ${booking._id?.toString?.() || booking.id || booking.bookingId || booking.rideId || ''}`;
    ...
    try {
      ...
      this.logger.log(`Email sent to driver ${driverEmail} for booking ${booking._id?.toString?.() || booking.id || booking.bookingId || booking.rideId}`);
      return true;
    } catch (err) {
  ...
  async sendDriverResponseEmailToUser(userEmail: string, userName: string, driverName: string, response: 'ACCEPTED' | 'REJECTED', booking: any) {
    ...
    const subject = response === 'ACCEPTED' ? `Driver confirmed your ride ${booking._id?.toString?.() || booking.id || booking.bookingId || booking.rideId}` : `Driver rejected your ride ${booking._id?.toString?.() || booking.id || booking.bookingId || booking.rideId}`;
